...
if [ -n "${GROQ_API_KEY:-}" ]; then
  echo "$RESULT" | jq -e '.severity_esi<=2 and (.citations|length)>=2 and (.suggested_labs|length)>=2' >/dev/null
else
  echo "no GROQ_API_KEY set — checking degraded-but-valid shape instead of live citations"
  echo "$RESULT" | jq -e '.severity_esi<=2 and (.red_flags|length)>=1' >/dev/null
fi
